...
import meteorclient.utils.player.DamageUtils;
...
public class CevBreaker extends Module {
    ...
    public CevBreaker() {
        super(Categories.BANANAPLUS, "cev-breaker", "Automatically places an obsidian block and a crystal on top of the target and breaks the obby and crystal to deal massive damage.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!blockState.isOf(Blocks.OBSIDIAN) && !crystalThere && (mc.player.getMainHandStack().getItem().equals(Items.OBSIDIAN) || switchDelayLeft <= 0)) {
            if (!BlockUtils.place(blockPos, InvUtils.findInHotbar(Items.OBSIDIAN), rotate.get(), 50, true)) {
        ...
        if (!(BBomber.isActive() && BBomber.centerDamage.get())) {

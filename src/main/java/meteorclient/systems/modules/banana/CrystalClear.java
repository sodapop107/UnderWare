...
public class CrystalClear extends Module {
    ...
    private final Setting<Object2BooleanMap<EntityType<?>>> entities = sgTargeting.add(new EntityTypeListSetting.Builder().name("entities").description("Entities to attack.").defaultValue(EntityType.END_CRYSTAL).onlyAttackable().build());
    ...
    private final Setting<SortPriority> priority = sgTargeting.add(new EnumSetting.Builder<SortPriority>().name("priority").description("How to filter targets within range.").defaultValue(SortPriority.HighestDistance).build());
    private final Setting<Integer> maxTargets = sgTargeting.add(new IntSetting.Builder().name("max-targets").description("How many entities to target at once.").defaultValue(1).min(1).max(10).sliderMin(1).sliderMax(5).build());
    ...
    public CrystalClear() {
        super(Categories.BANANAPLUS, "crystal-clear", "Attacks specified entities around you.");
    }
    ...
    private boolean entityCheck(Entity entity) {
        ...
        if (BPlayerUtils.distanceTo(entity) > range.get()) return false;
        if (!entities.get().getBoolean(entity.getType())) return false;
        if (!PlayerUtils.canSeeEntity(entity) && BPlayerUtils.distanceTo(entity) > wallsRange.get()) return false;

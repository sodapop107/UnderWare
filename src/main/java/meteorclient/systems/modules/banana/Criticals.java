...
import meteorclient.mixininterface.IPlayerMoveC2SPacket;
...
public class Criticals extends Module {
    ...
    private final Setting<Mode> mode = sgGeneral.add(new EnumSetting.Builder<Mode>().name("mode").description("The mode on how Criticals will function.").defaultValue(Mode.Packet).build());
    private final Setting<Boolean> ka = sgGeneral.add(new BoolSetting.Builder().name("only-killaura").description("Only performs crits when using killaura.").defaultValue(false).build());
    ...
    public Criticals() {
        super(Categories.BANANAPLUS, "criticals", "Performs critical attacks when you hit your target.");
    }

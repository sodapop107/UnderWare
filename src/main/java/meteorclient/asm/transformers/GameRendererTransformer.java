package meteorclient.asm.transformers;

import meteorclient.asm.AsmTransformer;
import meteorclient.asm.Descriptor;
import meteorclient.asm.FieldInfo;
import meteorclient.asm.MethodInfo;
...
public class GameRendererTransformer extends AsmTransformer {
    ...
    public void transform(ClassNode klass) {
        ...
        if (method == null) throw new RuntimeException("[UnderWare] Could not find method GameRenderer.getFov()");
        ...
        if (injectionCount < 2) throw new RuntimeException("[UnderWare] Failed to modify GameRenderer.getFov()");
    }
    ...
    private void generateEventCall(InsnList insns, AbstractInsnNode loadPreviousFov) {
        ...
        insns.add(new TypeInsnNode(Opcodes.CHECKCAST, "meteorclient/events/render/GetFovEvent"));
        insns.add(new FieldInsnNode(Opcodes.GETFIELD, "meteorclient/events/render/GetFovEvent", "fov", "D"));
    }
}
